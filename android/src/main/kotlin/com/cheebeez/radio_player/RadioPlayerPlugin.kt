...
class RadioPlayerPlugin : FlutterPlugin, MethodCallHandler {
    ...
    private lateinit var loadingChannel: EventChannel
    ...
    override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        ...
        loadingChannel = EventChannel(flutterPluginBinding.binaryMessenger, "radio_player/loadingEvents")
        loadingChannel.setStreamHandler(loadingStreamHandler)

        notificationChannel = EventChannel(flutterPluginBinding.binaryMessenger, "radio_player/notificationEvents")
    ...
    override fun onDetachedFromEngine(@NonNull binding: FlutterPlugin.FlutterPluginBinding) {
        ...
        service.isEngineAttached=false;
    ...
    /** Handler for loading state, passed to setStreamHandler() */
    private var loadingStreamHandler = object : StreamHandler {
        private var eventSink: EventSink? = null

        override fun onListen(arguments: Any?, events: EventSink?) {
            eventSink = events
            LocalBroadcastManager.getInstance(context).registerReceiver(broadcastReceiver,
                IntentFilter(RadioPlayerService.ACTION_NEW_LOADING_DATA))
        }
        ...
        private var broadcastReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                if (intent != null) {
                    val received = intent.getStringExtra(RadioPlayerService.ACTION_NEW_LOADING_DATA_EXTRA)
                    eventSink?.success(received)
                }
            }
        }
    ...
    /** Handler for loading state, passed to setStreamHandler() */
    private var notificationStreamHandler = object : StreamHandler {
        private var eventSink: EventSink? = null

        override fun onListen(arguments: Any?, events: EventSink?) {
            eventSink = events
            LocalBroadcastManager.getInstance(context).registerReceiver(broadcastReceiver,
                IntentFilter(RadioPlayerService.ACTION_NOTIFICATION_DATA))
        }
        ...
        private var broadcastReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                if (intent != null) {
                    val received = intent.getStringExtra(RadioPlayerService.ACTION_NOTIFICATION_EXTRA)
                    eventSink?.success(received)
                }
            }
        }
